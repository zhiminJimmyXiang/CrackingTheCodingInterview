...
class Chapter18{
 public:
    ...
    bool findSubsquares(const vector<vector<pair<int, int> > > &matrixPreknowledge, unsigned subsquareLen);
    ...
    int getSumValueOfRowSegment(const MATRIX &matrix, vector<vector<int> > &oneRowSumValues, int rowNum, int colStart, int colEnd);
